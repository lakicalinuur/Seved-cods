...
ADMIN_ID = 6964068910
WEBHOOK_URL = "https://distant-issy-wwmahe-868dbbd7.koyeb.app/"
...
# Gemini API key (used for translations and summaries — kept same approach as Bot1)
...
# Temporary in-memory store for recent transcriptions for translate/summarize follow-ups.
# Format: { user_id_str: { original_message_id: transcription_text } }
user_transcriptions = {}
...
def create_settings_reply_keyboard():
    markup = ReplyKeyboardMarkup(row_width=2, resize_keyboard=True)
    markup.add(
        KeyboardButton("Choose Voice"),
        KeyboardButton("Pitch"),
        KeyboardButton("Rate"),
        KeyboardButton("STT Lang"),
...
def start_handler(message):
    ...
    user_first_name = message.from_user.first_name if message.from_user.first_name else "Friend"
...
def help_handler(message):
    ...
    help_text = (
        ...
        "• Send text and get an audio file back\n\n"
        "2️⃣ *Audio to Text (STT)*\n"
        "• Choose *STT Lang* for accurate transcription\n"
        "• Send an audio message, audio file, or video (≤20MB)\n\n"
        "🔒 *Privacy*\n"
        "• Transcriptions and conversions are processed and not stored permanently.\n\n"
        "If you need help contact: @kookabeela"
    )
...
def handle_commands_command(message):
    ...
    if message.chat.type == 'private' and str(message.from_user.id) != str(ADMIN_ID) and not check_subscription(message.chat.id):
        send_subscription_message(message.chat.id)
        return
    bot.send_message(message.chat.id, "⚠️ The request you sent is currently unavailable.")
...
def handle_rate_command(message, target_bot: telebot.TeleBot, user_id_for_settings: str):
    ...
    target_bot.send_message(
        chat_id,
        "🔧 *Speech Rate*\nChoose a preset or send a number from -100 (slow) to +100 (fast). 0 = normal.",
...
def handle_rate_callback(call, target_bot: telebot.TeleBot, user_id_for_settings: str):
    ...
    try:
        ...
        target_bot.answer_callback_query(call.id, f"Rate set to {rate_value}")
        target_bot.edit_message_text(
            chat_id=chat_id,
            message_id=call.message.message_id,
            text=f"🔊 Speech rate is *{rate_value}*.\n\nSend text to convert it to audio or open Settings to change voice.",
            parse_mode="Markdown",
            reply_markup=None
        )
    except ValueError:
...
def handle_pitch_command(message, target_bot: telebot.TeleBot, user_id_for_settings: str):
    ...
    target_bot.send_message(
        chat_id,
        "🎚️ *Pitch*\nChoose a preset or send a number from -100 (low) to +100 (high). 0 = normal.",
...
def handle_voice_command(message, target_bot: telebot.TeleBot, user_id_for_settings: str):
    ...
    user_pitch_input_mode[user_id_for_settings] = None
    user_rate_input_mode[user_id_for_settings] = None
    target_bot.send_message(chat_id, "🌐 Choose the language for available voices:", reply_markup=make_tts_language_keyboard(), parse_mode="Markdown")
...
def handle_tts_language_select_callback(call, target_bot: telebot.TeleBot, user_id_for_settings: str):
    ...
    target_bot.edit_message_text(
        chat_id=chat_id,
        message_id=call.message.message_id,
        text=f"🗣️ Now select a voice from *{lang_name}*:",
...
def handle_tts_voice_change_callback(call, target_bot: telebot.TeleBot, user_id_for_settings: str):
    ...
    target_bot.answer_callback_query(call.id, f"✔️ Voice set: {voice_display_name}")
    target_bot.edit_message_text(
        chat_id=chat_id,
        message_id=call.message.message_id,
        text=f"🔊 *{voice_display_name}* selected.\n\nCurrent settings:\n• Pitch: *{current_pitch}*\n• Rate: *{current_rate}*\n\nSend text to generate audio.",
        parse_mode="Markdown",
        reply_markup=None
    )
...
def handle_tts_back_to_languages_callback(call, target_bot: telebot.TeleBot, user_id_for_settings: str):
    ...
    target_bot.edit_message_text(
        chat_id=chat_id,
        message_id=call.message.message_id,
        text="🔁 Choose the language of the voice:",
...
async def synth_and_send_tts(chat_id: int, user_id_for_settings: str, text: str, target_bot: telebot.TeleBot):
    ...
    try:
        ...
        if not os.path.exists(filename) or os.path.getsize(filename) == 0:
            target_bot.send_message(chat_id, "❌ Failed to create audio. Try different text or voice.")
            return
        ...
        target_bot.send_message(chat_id, "❌ An error occurred while generating the audio. Try another voice.")
    except Exception as e:
        logging.exception("TTS error")
        target_bot.send_message(chat_id, "❌ This voice is not available. Choose another voice.")
    finally:
        stop_recording.set()
...
def handle_language_stt_command(message, target_bot: telebot.TeleBot, user_id_for_settings: str):
    ...
    user_pitch_input_mode[user_id_for_settings] = None
    user_rate_input_mode[user_id_for_settings] = None
    target_bot.send_message(chat_id, "🌍 Choose the transcription language:", reply_markup=build_stt_language_keyboard(), parse_mode="Markdown")
...
def handle_stt_language_select_callback(call, target_bot: telebot.TeleBot, user_id_for_settings: str):
    ...
    target_bot.answer_callback_query(call.id, f"✅ Language set: {lang_name}")
    target_bot.edit_message_text(
        chat_id=chat_id,
        message_id=call.message.message_id,
        text=f"✅ Transcription language: *{lang_name}*\n\nSend audio, file, or video (≤20MB) to transcribe.",
        parse_mode="Markdown",
        reply_markup=None
    )
...
async def process_stt_media(chat_id: int, user_id_for_settings: str, message_type: str, file_id: str, target_bot: telebot.TeleBot, original_message_id: int):
    ...
    try:
        ...
        if res['status'] == 'completed':
            text = res.get("text", "")
            if not text:
                target_bot.send_message(chat_id, "ℹ️ No text was extracted from this file.", reply_to_message_id=original_message_id)
            else:
                ...
                # Store transcription temporarily for follow-up actions (10-minute TTL)
                uid_str = str(user_id_for_settings)
                user_transcriptions.setdefault(uid_str, {})[original_message_id] = text
                ...
                def delete_transcription_later(u_id, msg_id):
                    ...
                    try:
                        ...
                        logging.error(f"Error deleting transcription later: {e}")

                threading.Thread(target=delete_transcription_later, args=(uid_str, original_message_id), daemon=True).start()

                if len(text) <= 4000:
                    # Send text with inline buttons
                    target_bot.send_message(chat_id, text, reply_to_message_id=original_message_id, reply_markup=buttons)
                else:
                    # Send as document and attach buttons
                    import io
                    fbuf = io.BytesIO(text.encode("utf-8"))
                    fbuf.name = f"transcript_{uuid.uuid4()}.txt"
                    target_bot.send_document(chat_id, fbuf, caption="Transcription (too long for chat).", reply_to_message_id=original_message_id, reply_markup=buttons)
                    fbuf.close()
        ...
        target_bot.send_message(chat_id, "😓 I can’t transcribe files larger than 20 MB. Please send a smaller one.", reply_to_message_id=original_message_id)
    finally:
        if processing_msg:
            try:
...
def handle_text_for_tts_or_mode_input_common(message, target_bot: telebot.TeleBot, user_id_for_settings: str):
    ...
    if message.text == "🗣️ Text to voice":
        target_bot.send_message(message.chat.id, "📝 Send the text you'd like converted to audio.")
        return
    if user_rate_input_mode.get(user_id_for_settings) == "awaiting_rate_input":
        try:
            ...
            if -100 <= rate_val <= 100:
                ...
                target_bot.send_message(message.chat.id, f"🔊 Speech rate set to *{rate_val}*", parse_mode="Markdown")
                user_rate_input_mode[user_id_for_settings] = None
            else:
                target_bot.send_message(message.chat.id, "❌ Invalid rate. Send a number between -100 and +100.")
            return
        except ValueError:
            target_bot.send_message(message.chat.id, "❌ Not a valid number. Send -100 to +100.")
            return
    if user_pitch_input_mode.get(user_id_for_settings) == "awaiting_pitch_input":
        try:
            ...
            if -100 <= pitch_val <= 100:
                ...
                target_bot.send_message(message.chat.id, f"🎚️ Pitch set to *{pitch_val}*", parse_mode="Markdown")
                user_pitch_input_mode[user_id_for_settings] = None
            else:
                target_bot.send_message(message.chat.id, "❌ Invalid pitch. Send a number between -100 and +100.")
            return
        except ValueError:
            target_bot.send_message(message.chat.id, "❌ Not a valid number. Send -100 to +100.")
            return
    ...
    if current_voice:
        ...
        target_bot.send_message(
            message.chat.id,
            "⚙️ You haven't chosen a voice yet. Open Settings → Choose Voice, then send text."
        )
...
def handle_settings_menu(message):
    ...
    if message.text == "⚙️ Settings":
        bot.send_message(message.chat.id, "⚙️ *Settings*\nChoose the setting you'd like to adjust:", reply_markup=create_settings_reply_keyboard(), parse_mode="Markdown")
    elif message.text == "Main Menu":
        bot.send_message(message.chat.id, "🏠 Returned to the main menu.", reply_markup=create_main_reply_keyboard())
...
def handle_unsupported_media_types(message):
    ...
    bot.send_message(
        message.chat.id,
        "⚠️ I can only convert text to audio or audio/video to text. Send one of those and I'll help you."
    )

bot_start_time = datetime.now()

# ------------------
# Gemini helper (for translate/summarize) — same approach as Bot1
# ------------------
def ask_gemini(user_id, user_message):
    """
    Send conversation history to Gemini and return the response text.
    """
    # Keep a tiny per-user memory in in_memory_data (or could keep a separate dictionary)
    # We'll keep it simple: send just the user_message with a system instruction.
    system_instruction_part = {"text": "For translations, use only a comma (,) as a pause symbol. Do not use a period (.)."}
    parts = [system_instruction_part, {"text": user_message}]
    url = (
...
# ------------------
# Translate / Summarize inline flows
# ------------------
...
def button_translate_handler(call):
    ...
    try:
        _, orig_msg_id_str = call.data.split("|", 1)
    ...
    if uid not in user_transcriptions or orig_msg_id not in user_transcriptions[uid]:
        bot.answer_callback_query(call.id, "❌ No transcription found for this message (maybe expired).")
        return

    # Build language selection keyboard for translation: use STT_LANGUAGES keys but will send codes to action.
    markup = InlineKeyboardMarkup(row_width=3)
    buttons = []
    for lang_display, lang_code in STT_LANGUAGES.items():
        buttons.append(InlineKeyboardButton(lang_display, callback_data=f"translate_to|{lang_code}|{orig_msg_id}"))
    for i in range(0, len(buttons), 3):
        markup.add(*buttons[i:i+3])
    ...
    bot.edit_message_text(
        chat_id=call.message.chat.id,
        message_id=call.message.message_id,
        text="Please choose the target language for translation:",
        reply_markup=markup
    )
    bot.answer_callback_query(call.id)
...
def callback_translate_to(call):
    ...
    if len(parts) < 3:
        ...
        return
    _, lang_code, orig_msg_id_str = parts
    orig_msg_id = int(orig_msg_id_str)
    ...
    original_text = user_transcriptions[uid][orig_msg_id]
    # Build prompt — use language code mapping to display name if possible
    lang_display_name = next((k for k, v in STT_LANGUAGES.items() if v == lang_code), lang_code)
    prompt = (
        f"Translate the following text into {lang_display_name}. Provide only the translated text, "
    ...
    # Call Gemini in background thread to avoid blocking
    def do_translate():
        translated = ask_gemini(uid, prompt)
        if translated.startswith("Error:"):
            bot.send_message(call.message.chat.id, f"😓 Sorry, an error occurred during translation: {translated}.")
            return
        if len(translated) <= 4000:
            bot.send_message(call.message.chat.id, translated, reply_to_message_id=orig_msg_id)
        else:
            import io
            buf = io.BytesIO(translated.encode("utf-8"))
            buf.name = f"translation_{uuid.uuid4()}.txt"
            bot.send_document(call.message.chat.id, buf, caption=f"Translation to {lang_display_name}", reply_to_message_id=orig_msg_id)
            buf.close()
...
def button_summarize_handler(call):
    ...
    # Build language selection keyboard for summary (no 'Auto' button as per your request)
    markup = InlineKeyboardMarkup(row_width=3)
    buttons = []
    for lang_display, lang_code in STT_LANGUAGES.items():
        buttons.append(InlineKeyboardButton(lang_display, callback_data=f"summarize_in|{lang_code}|{orig_msg_id}"))
    for i in range(0, len(buttons), 3):
        markup.add(*buttons[i:i+3])
    ...
    bot.edit_message_text(
        chat_id=call.message.chat.id,
        message_id=call.message.message_id,
        text="Please choose the target language for the summary:",
        reply_markup=markup
    )
    bot.answer_callback_query(call.id)
...
def callback_summarize_in(call):
    ...
    prompt = (
        f"Summarize the following text in {lang_display_name}. Provide only the summarized text, "
    ...
    def do_summarize():
        ...
        if summary.startswith("Error:"):
            bot.send_message(call.message.chat.id, f"😓 Sorry, an error occurred during summarization: {summary}.")
            return
        if len(summary) <= 4000:
            bot.send_message(call.message.chat.id, summary, reply_to_message_id=orig_msg_id)
        else:
            import io
            buf = io.BytesIO(summary.encode("utf-8"))
            buf.name = f"summary_{uuid.uuid4()}.txt"
            bot.send_document(call.message.chat.id, buf, caption=f"Summary in {lang_display_name}", reply_to_message_id=orig_msg_id)
            buf.close()

    threading.Thread(target=do_summarize, daemon=True).start()

def handle_stt_media_types_common(message, target_bot: telebot.TeleBot, user_id_for_settings: str):
    # (This function is defined above already — keep original usage)
    pass  # placeholder (function already defined earlier)
...
# ------------------
# /status implementation (uses in-memory counters)
# ------------------
...
def status_handler(message):
    ...
    # Active today count (from in_memory_data users)
    today = datetime.now().date()
    active_today_count = 0
    for u in in_memory_data["users"].values():
        la = u.get("last_active")
        if isinstance(la, datetime) and la.date() == today:
    ...
    total_registered_users = len(in_memory_data["users"]) if in_memory_data["users"] else users_collection.count_documents({})

    # Sum STT/TTS counts across all users (in-memory)
    total_tts = sum(u.get("tts_conversion_count", 0) for u in in_memory_data["users"].values())
    total_stt = sum(u.get("stt_conversion_count", 0) for u in in_memory_data["users"].values())
    ...
    text = (
        ...
        f"▫️ Total Media Files Processed (STT): {total_stt}\n"
        f"▫️ Total Text-to-Speech Conversions (TTS): {total_tts}\n\n"
        "⸻\n\n"
