...
GEMINI_API_KEY = "AIzaSyCHrGhRKXAp3DuQGH8HLB60ggryZeUFA9E"  # Added Gemini API key
...
user_transcriptions = {}  # Added to store transcriptions temporarily
...
TTS_VOICES_BY_LANGUAGE = {
 "Arabic": ["ar-DZ-AminaNeural", "ar-DZ-IsmaelNeural", "ar-BH-AliNeural", "ar-BH-LailaNeural", "ar-EG-SalmaNeural", "ar-EG-ShakirNeural", "ar-IQ-BasselNeural", "ar-IQ-RanaNeural", "ar-JO-SanaNeural", "ar-JO-TaimNeural", "ar-KW-FahedNeural", "ar-KW-NouraNeural", "ar-LB-LaylaNeural", "ar-LB-RamiNeural", "ar-LY-ImanNeural", "ar-LY-OmarNeural", "ar-MA-JamalNeural", "ar-MA-MounaNeural", "ar-OM-AbdullahNeural", "ar-OM-AyshaNeural", "ar-QA-AmalNeural", "ar-QA-MoazNeural", "ar-SA-HamedNeural", "ar-SA-ZariyahNeural", "ar-SY-AmanyNeural", "ar-SY-LaithNeural", "ar-TN-HediNeural", "ar-TN-ReemNeural", "ar-AE-FatimaNeural", "ar-AE-HamdanNeural", "ar-YE-MaryamNeural", "ar-YE-SalehNeural"],
    ...
    # ... (rest truncated for brevity in this excerpt; full mapping is included above)
}
...
# --- Helper functions for large text chunking + processing ---
def chunk_text(text: str, max_chars: int = 25000):
    """Split text into chunks of up to max_chars (simple slicing)."""
    chunks = []
    start = 0
...
def translate_large_text_with_gemini(text: str, target_lang_name: str):
    """Translate large text by chunking, translating each chunk, then combining and polishing."""
...
def summarize_large_text_with_gemini(text: str, target_lang_name: str):
    """Summarize large text by chunking, summarizing each chunk, then combining and polishing."""
    chunks = chunk_text(text, max_chars=24000)
    partial_summaries = []
    for i, chunk in enumerate(chunks):
...
# Added to delete transcriptions after 10 minutes
    ...
    try:
        ...
        if res['status'] == 'completed':
            ...
            if not text:
                ...
                # Store transcription for later use in translate/summarize
                user_transcriptions.setdefault(user_id_for_settings, {})[original_message_id] = text
                # Schedule deletion after 10 minutes
                threading.Thread(
                    target=delete_transcription_later,
                    args=(user_id_for_settings, original_message_id),
                    daemon=True
                ).start()
                
                # Add Translate and Summarize buttons
                markup = InlineKeyboardMarkup()
                markup.add(
                ...
                # --- NEW: store the long transcription and attach Translate/Summarize buttons ---
                user_transcriptions.setdefault(user_id_for_settings, {})[original_message_id] = text
                # Schedule deletion after 10 minutes
                threading.Thread(
                    target=delete_transcription_later,
                ...
                # Build Translate / Summarize buttons that reference the original message id
                markup = InlineKeyboardMarkup()
                markup.add(
                    InlineKeyboardButton("Translate", callback_data=f"btn_translate|{original_message_id}"),
                    InlineKeyboardButton("Summarize", callback_data=f"btn_summarize|{original_message_id}")
                ...
                # send the transcript as a downloadable .txt file + include the buttons
                ...
                try:
                    target_bot.send_document(
                        chat_id,
                        f,
                        caption="The transcription was too long to send as a message; use the buttons below to Translate or Summarize.",
                        reply_to_message_id=original_message_id,
                        reply_markup=markup
                    )
                except Exception as e:
                    logging.error(f"Failed to send transcript file with buttons: {e}")
                    # fallback: send file without markup
                    try:
...
# Added callback handlers for translate/summarize buttons
...
def button_translate_handler(call):
    ...
    try:
        ...
        # If editing fails (e.g., message is a document preview), just send a new message with options
...
def button_summarize_handler(call):
    ...
    # Show language selection for summarization
    markup = InlineKeyboardMarkup(row_width=3)
    buttons = []
    for lang_name in STT_LANGUAGES.keys():
    ...
    try:
        bot.edit_message_text(
            chat_id=call.message.chat.id,
            message_id=call.message.message_id,
            text="🌍 Select language for summary:",
            reply_markup=markup
        )
    except Exception:
...
# Updated handler for translation language selection (now runs processing in a separate thread)
...
def callback_translate_to(call):
    ...
    # Notify user and spawn worker thread
    try:
        bot.edit_message_text(
            chat_id=call.message.chat.id,
            message_id=call.message.message_id,
    ...
    def do_translate():
        ...
        if translated.startswith("Error:"):
            ...
            return
        # If translation is long, send as file
...
# Updated handler for summarization language selection (now runs processing in a separate thread)
...
def callback_summarize_in(call):
    ...
    try:
        bot.edit_message_text(
            chat_id=call.message.chat.id,
            message_id=call.message.message_id,
            text=f"🔄 Summarizing in {lang_name}...\nPlease wait — processing long transcript."
        )
    except Exception:
        bot.send_message(call.message.chat.id, f"🔄 Summarizing in {lang_name}... Please wait — processing long transcript.")
    bot.answer_callback_query(call.id)
