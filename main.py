...
def send_subscription_message(chat_id: int):
    if bot.get_chat(chat_id).type == 'private':
        ...
        bot.send_message(
            chat_id,
            "🔒 Please join our channel to use the bot. Tap below and send /start.",
            reply_markup=markup
        )
...
def start_handler(message):
    ...
    if message.from_user.id == ADMIN_ID:
        ...
        welcome_message = (
            ...
            "AI Voice Assistant\n\n"
            "• Settings → pick voice, pitch, rate\n"
            "• Send text → get audio\n"
            "• Send audio/video → get transcript\n\n"
            "Add to groups: https://t.me/mediatotextbot?startgroup="
        )
...
def help_handler(message):
    ...
    help_text = (
        "⚡ Quick Help\n\n"
        "TTS: Settings → Choose Voice → send text\n"
        "STT: Settings → STT Lang → send audio/video (≤20MB)\n\n"
        "Data: used only for processing. Preferences saved.\n\n"
        "Support: @kookabeela"
    )
...
def privacy_notice_handler(message):
    ...
    privacy_text = (
        "🔐 Privacy\n\n"
        "Audio and text are processed temporarily. Contact @user33230 for questions."
    )
...
def handle_commands_command(message):
    ...
    if message.chat.type == 'private' and str(message.from_user.id) != str(ADMIN_ID) and not check_subscription(message.from_user.id):
        send_subscription_message(message.chat.id)
        return
    bot.send_message(message.chat.id, "This command is unavailable.")
...
def admin_menu_callback(call):
    ...
    if call.data == "admin_total_users":
        ...
        bot.send_message(chat_id, f"Total users: {total_registered}")
    elif call.data == "admin_broadcast":
        admin_state[call.from_user.id] = 'awaiting_broadcast_message'
        bot.send_message(chat_id, "Send the broadcast now:")
...
def make_pitch_keyboard():
    ...
    markup.add(
        ...
        InlineKeyboardButton("⬇️ Low", callback_data="pitch_set|-50"),
...
def make_rate_keyboard():
    markup = InlineKeyboardMarkup(row_width=2)
    markup.add(
        InlineKeyboardButton("⚡️ Fast", callback_data="rate_set|+50"),
...
def handle_rate_callback(call, target_bot: telebot.TeleBot, user_id_for_settings: str):
    ...
    try:
        ...
        target_bot.answer_callback_query(call.id, f"Rate: {rate_value}")
        target_bot.edit_message_text(
            chat_id=chat_id,
            message_id=call.message.message_id,
            text=f"🔊 Rate: *{rate_value}*.\nSend text.",
            parse_mode="Markdown",
            reply_markup=None
        )
    except ValueError:
        ...
        target_bot.answer_callback_query(call.id, "Error.")
...
def handle_pitch_callback(call, target_bot: telebot.TeleBot, user_id_for_settings: str):
    ...
    try:
        ...
        target_bot.answer_callback_query(call.id, f"Pitch: {pitch_value}")
        target_bot.edit_message_text(
            chat_id=chat_id,
            message_id=call.message.message_id,
            text=f"🔊 Pitch: *{pitch_value}*.\nSend text.",
            parse_mode="Markdown",
            reply_markup=None
        )
    except ValueError:
...
def handle_voice_command(message, target_bot: telebot.TeleBot, user_id_for_settings: str):
    ...
    user_pitch_input_mode[user_id_for_settings] = None
    user_rate_input_mode[user_id_for_settings] = None
    target_bot.send_message(chat_id, "Pick voice language:", reply_markup=make_tts_language_keyboard(), parse_mode="Markdown")
...
def handle_tts_language_select_callback(call, target_bot: telebot.TeleBot, user_id_for_settings: str):
    ...
    target_bot.edit_message_text(
        chat_id=chat_id,
        message_id=call.message.message_id,
        text=f"Choose a voice from {lang_name}:",
...
def handle_tts_voice_change_callback(call, target_bot: telebot.TeleBot, user_id_for_settings: str):
    ...
    target_bot.answer_callback_query(call.id, f"Voice: {voice_display_name}")
    target_bot.edit_message_text(
        chat_id=chat_id,
        message_id=call.message.message_id,
        text=f"✅ {voice_display_name}\nPitch: {current_pitch}  Rate: {current_rate}\nSend text.",
        parse_mode="Markdown",
        reply_markup=None
    )
...
async def synth_and_send_tts(chat_id: int, user_id_for_settings: str, text: str, target_bot: telebot.TeleBot):
    ...
    try:
        ...
        if not os.path.exists(filename) or os.path.getsize(filename) == 0:
            target_bot.send_message(chat_id, "❌ Audio generation failed.")
            return
        with open(filename, "rb") as f:
            voice_display_name = VOICE_MAPPING.get(voice, voice)
            caption_text = f"🎧 {voice_display_name}\nPitch: {pitch}  Rate: {rate}"
        ...
        target_bot.send_message(chat_id, "❌ Error generating audio.")
    except Exception as e:
        logging.exception("TTS error")
        target_bot.send_message(chat_id, "❌ Voice unavailable.")
    finally:
        stop_recording.set()
...
def handle_language_stt_command(message, target_bot: telebot.TeleBot, user_id_for_settings: str):
    ...
    user_pitch_input_mode[user_id_for_settings] = None
    user_rate_input_mode[user_id_for_settings] = None
    target_bot.send_message(chat_id, "Select STT language:", reply_markup=build_stt_language_keyboard(), parse_mode="Markdown")
...
def handle_stt_language_select_callback(call, target_bot: telebot.TeleBot, user_id_for_settings: str):
    ...
    target_bot.answer_callback_query(call.id, f"Language: {lang_name}")
    target_bot.edit_message_text(
        chat_id=chat_id,
        message_id=call.message.message_id,
        text=f"✅ {lang_name}\nSend audio/video (≤20MB).",
        parse_mode="Markdown",
        reply_markup=None
    )
...
async def process_stt_media(chat_id: int, user_id_for_settings: str, message_type: str, file_id: str, target_bot: telebot.TeleBot, original_message_id: int):
    ...
    try:
        ...
        if not transcript_id:
            raise Exception("No transcript ID.")
        ...
        if res['status'] == 'completed':
            text = res.get("text", "")
            if not text:
                target_bot.send_message(chat_id, "ℹ️ No text found.", reply_to_message_id=original_message_id)
            elif len(text) <= 4000:
                ...
                target_bot.send_document(chat_id, f, caption="Transcript:", reply_to_message_id=original_message_id)
            increment_processing_count_in_memory(user_id_for_settings, "stt")
        else:
            error_msg = res.get("error", "Transcription error.")
            target_bot.send_message(chat_id, f"❌ {error_msg}", parse_mode="Markdown", reply_to_message_id=original_message_id)
        ...
        target_bot.send_message(chat_id, "❌ Failed to process file. Try a smaller one.", reply_to_message_id=original_message_id)
    finally:
        if processing_msg:
            try:
...
def handle_stt_media_types_common(message, target_bot: telebot.TeleBot, user_id_for_settings: str):
    ...
    if message.voice:
        ...
        if message.document.mime_type and (message.document.mime_type.startswith('audio/') or message.document.mime_type.startswith('video/')):
            ...
            target_bot.send_message(message.chat.id, "I only transcribe audio/video. Send a valid file.")
            return
    if not file_id:
        target_bot.send_message(message.chat.id, "Unsupported file type.")
        return
    if user_id_for_settings not in in_memory_data["stt_settings"]:
        target_bot.send_message(message.chat.id, "Choose transcription language first (/lang).")
        return
    threading.Thread(
...
def handle_text_for_tts_or_mode_input_common(message, target_bot: telebot.TeleBot, user_id_for_settings: str):
    ...
    if message.text == "🗣️ Text to voice":
        target_bot.send_message(message.chat.id, "Send the text to convert.")
        return
    if user_rate_input_mode.get(user_id_for_settings) == "awaiting_rate_input":
        try:
            ...
            if -100 <= rate_val <= 100:
                ...
                target_bot.send_message(message.chat.id, f"🔊 Rate set to *{rate_val}*.", parse_mode="Markdown")
                user_rate_input_mode[user_id_for_settings] = None
            else:
                target_bot.send_message(message.chat.id, "Invalid rate. Send -100..+100.")
            return
        except ValueError:
            target_bot.send_message(message.chat.id, "Send a valid number -100..+100.")
            return
    if user_pitch_input_mode.get(user_id_for_settings) == "awaiting_pitch_input":
        try:
            ...
            if -100 <= pitch_val <= 100:
                ...
                target_bot.send_message(message.chat.id, "Invalid pitch. Send -100..+100.")
            return
        except ValueError:
            target_bot.send_message(message.chat.id, "Send a valid number -100..+100.")
            return
    ...
    if current_voice:
        ...
        target_bot.send_message(
            message.chat.id,
            "No voice set. Go to Settings → Choose Voice."
        )
...
def handle_settings_menu(message):
    ...
    if message.text == "⚙️ Settings":
        bot.send_message(message.chat.id, "Settings:", reply_markup=create_settings_reply_keyboard())
    elif message.text == "Main Menu":
        bot.send_message(message.chat.id, "Main Menu:", reply_markup=create_main_reply_keyboard())
...
def handle_unsupported_media_types(message):
    ...
    bot.send_message(
        message.chat.id,
        "I only convert text↔audio. Send text or an audio/video file."
    )
...
def set_bot_commands():
    commands = [
        ...
        BotCommand("lang", "🌍 Set STT language"),
...
def set_bot_info_and_startup():
    ...
    if check_db_connection():
        ...
        logging.error("Failed to connect to MongoDB. Running with in-memory data only.")
        set_webhook_on_startup()
        set_bot_commands()
