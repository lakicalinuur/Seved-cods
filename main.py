...
# --- (Your imports and global variables remain the same) ---
logging.basicConfig(
    level=logging.INFO,
...
# --- New function to create the reply keyboard ---
...
def start_handler(message):
    ...
    if message.from_user.id == ADMIN_ID:
        ...
        bot.send_message(
            message.chat.id,
            welcome_message,
            reply_markup=create_main_reply_keyboard(), # Use the new reply keyboard here
            parse_mode="Markdown"
        )
...
def handle_commands_command(message):
    # This command is no longer needed but kept to handle legacy use,
    # as the new commands are handled by reply buttons.
    update_user_activity_in_memory(message.from_user.id)
    if message.chat.type == 'private' and str(message.from_user.id) != str(ADMIN_ID) and not check_subscription(message.from_user.id):
        send_subscription_message(message.chat.id)
        return
...
def handle_text_for_tts_or_mode_input_common(message, target_bot: telebot.TeleBot, user_id_for_settings: str):
    ...
    if current_voice:
        ...
        target_bot.send_message(
            message.chat.id,
            "You haven't chosen a voice yet! Use the `Choose Voice` button from `Settings` first, then send me text. 🗣️"
        )
...
def set_bot_commands():
    # Removed the /commands command as per user request
    commands = [
...
# --- NEW: A dedicated endpoint to manually set commands and webhook ---
...
if __name__ == "__main__":
    # --- The existing startup logic remains here for initial setup attempts ---
